...
function Item(unserialize = {}, onUpdate, onDelete, onEscape, onDragStart) {
    ...
    item = elem()
        .classes("todo-item", "flex", "gap-10", "items-center")
        .append(
            elem("input")
    ...
    return fragment().append(
        (inputContainer = elem()
            ...
            .append(
                ...
                (input = autoScalingTextarea(
                    (textarea) =>
                        (inputArea = textarea.on("keydown", handleInputKeyDown).attrs({
...
export function verticallyReorderable(
    itemsContainer,
    onItemRepositioned,
    onDragEnd
) {
    ...
    const handleReposition = (event) => {
        ...
        const offsetY =
            client.y - container.initialRect.y - item.clientOffset.y + scrollOffset;
